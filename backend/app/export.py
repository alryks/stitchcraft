...
def printable_html(pattern: dict) -> str:
    ...
    stitch_at = {(s["x"], s["y"]): s for s in pattern["stitches"]}
    cells = []
    for y in range(pattern["height"]):
        for x in range(pattern["width"]):
            stitch = stitch_at.get((x, y))
            if stitch is None:
    ...
    stitch_legend = '''<li><b class="stitch-mark full">A</b> Полный крест, символ обозначает цвет нити</li>
<li><b class="stitch-mark half-forward"></b> Полукрест ↘</li>
<li><b class="stitch-mark half-backward"></b> Полукрест ↗</li>
<li><b class="stitch-mark blend"></b> Смешанный крест: второй цвет по диагонали</li>'''
